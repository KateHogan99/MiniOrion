...
{
    StateDerivative spacecraft_derivative;

    spacecraft_derivative.position_derivative = spacecraft.velocity;
    spacecraft_derivative.velocity_derivative = add_vectors(
        thrust_acceleration, 
        calculate_gravity_acceleration(spacecraft.position, mu)
    );

    return spacecraft_derivative;
}
...
State euler_integrate(State current_state, StateDerivative state_derivative, double d_time)
{
    State new_state;

    new_state.position = add_vectors(
        current_state.position,
        scale_vector(state_derivative.position_derivative, d_time)
    );
    new_state.velocity = add_vectors(
        current_state.velocity,
        scale_vector(state_derivative.velocity_derivative, d_time)
    );
...
{
    StateDerivative spacecraft_derivative = calculate_state_derivative(spacecraft, thrust_acceleration, mu);
    return euler_integrate(spacecraft, spacecraft_derivative, d_time);
}
...
double calculate_vehicle_mass(Vehicle vehicle)
{
    return vehicle.dry_mass + vehicle.propellant_mass;
}
...
SimulationConfig test_no_gravity()
{
    ...
    config.number_of_steps = simulation_duration / config.time_step;
...
void run_simulation(SimulationConfig config)
{
    ...
    {
        throw std::runtime_error(
            "Invalid initial condition: spacecraft cannot start at the origin when gravity is enabled."
        );
    }
    ...
    for (int step = 1; step <= config.number_of_steps; step++)
    {
        ...
        State spacecraft_t = propagate_state(spacecraft, config.thrust_acceleration, config.mu, config.time_step);

        double distance_this_step = calculate_distance_this_step(previous_state.position, spacecraft_t.position);
        total_distance += distance_this_step;

        Vector2D gravity_acceleration = calculate_gravity_acceleration(previous_state.position, config.mu);
        Vector2D total_acceleration = add_vectors(gravity_acceleration, config.thrust_acceleration);

        double elapsed_time = step * config.time_step;

        std::cout << "Time: " << elapsed_time << "\n";

        std::cout << "Position: " << spacecraft_t.position.x << ", " << spacecraft_t.position.y << "\n";

        std::cout << "Velocity: " << spacecraft_t.velocity.x << ", " << spacecraft_t.velocity.y << "\n";

        std::cout << "Speed: " << calculate_magnitude(spacecraft_t.velocity) << "\n";

        std::cout << "Gravity acceleration: " << gravity_acceleration.x << ", " << gravity_acceleration.y << "\n";

        std::cout << "Thrust acceleration: " << config.thrust_acceleration.x << ", " << config.thrust_acceleration.y << "\n";

        std::cout << "Total acceleration: " << total_acceleration.x << ", " << total_acceleration.y << "\n";

        std::cout << "Distance this step: " << distance_this_step << "\n";
    ...
    double final_displacement = calculate_displacement(initial_state.position, spacecraft.position);

    std::cout << "========================================\n";
    std::cout << "SIMULATION SUMMARY\n";
    std::cout << "========================================\n";

    std::cout << "Initial position: " << initial_state.position.x << ", " << initial_state.position.y << "\n";

    std::cout << "Final position: " << spacecraft.position.x << ", " << spacecraft.position.y << "\n";

    std::cout << "Initial velocity: " << initial_state.velocity.x << ", " << initial_state.velocity.y << "\n";

    std::cout << "Final velocity: " << spacecraft.velocity.x << ", " << spacecraft.velocity.y << "\n";

    std::cout << "Final displacement: " << final_displacement << "\n";

    std::cout << "Total distance traveled: " << total_distance << "\n";
}
